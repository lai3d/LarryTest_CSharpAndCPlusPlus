...
namespace _15_Attribute_GetHashCode
{
    class AttributeTest
    {
        static void Main(string[] args)
        {
            ...
            foreach (var method in clsType.GetMethods())
            {
                ...
                AuthorsAttribute authAttr = (AuthorsAttribute)Attribute.GetCustomAttribute( method, typeof(AuthorsAttribute) );
                if (authAttr != null)
                    // Add the information to the author list.
                    authorsInfo.Add(Tuple.Create(clsType.Name + "." + method.Name, authAttr));
            }
            ...
            for (int ctr = 0; ctr < authorsInfo.Count; ctr++)
            {
                ...
                if (!listed[ctr])
                {
                    Console.WriteLine("{0} and {1}", authorInfo.Item2.AuthorName1, authorInfo.Item2.AuthorName2);
                    listed[ctr] = true;
